package com.asher.convexhulls;

import javafx.animation.KeyFrame;
import javafx.animation.Timeline;
import javafx.scene.Group;
import javafx.scene.paint.Color;
import javafx.scene.shape.Line;
import javafx.stage.Stage;
import javafx.util.Duration;

import java.util.Comparator;
import java.util.List;
import java.util.Stack;

public class animGrahamScan {
    static int maxY;
    static int maxIdx = -1;
    static point maxPoint;
    static Stack<point> stack = new Stack<>();
    static point last;
    static point secondLast;
    static Stack<Line> lines = new Stack<>();
    static Timeline tl = new Timeline();
    static Timeline tl2 = new Timeline();
    static int i = 0;
    static int j = 2;
    static boolean shouldCont = false;

    private static void findMaxY(List<point> points) {
        maxY = points.get(0).y;
        maxIdx = 0;
        maxPoint = points.get(0);
        for (int i = 1; i < points.size(); i++) {
            if (points.get(i).y > maxY) {
                maxY = points.get(i).y;
                maxIdx = i;
                maxPoint = points.get(i);
            }
        }
    }

    public static void grahamScan(List<point> points, Stage stg, Group grp) {
        tl.setCycleCount(Timeline.INDEFINITE);
        tl2.setCycleCount(Timeline.INDEFINITE);
        stg.getScene().setOnKeyPressed(null);
        findMaxY(points);
        points.set(maxIdx, points.get(0));
        points.set(0, maxPoint);
        KeyFrame kf = new KeyFrame(Duration.seconds(0.1), actionEvent -> {
            if (!lines.isEmpty()) grp.getChildren().remove(lines.pop());
            points.get(i).angle = ConvexHullUtil.myAngle(points.get(0), points.get(i));
            drawLine(points.get(0), points.get(i), grp);
            i++;
            if (i >= points.size()) {
                grp.getChildren().remove(lines.pop());
                grahamScanPart2(points, stg, grp);
                tl.stop();
            }
        });
        tl.getKeyFrames().add(kf);
        tl.play();
    }

    private static void grahamScanPart2(List<point> points, Stage stg, Group grp) {
        lines.clear();
        points.sort(Comparator.comparingDouble(point::getAngle));
        stack.push(points.get(0));
        secondLast = points.get(0);
        stack.push(points.get(1));
        last = points.get(1);
        drawLine(last, secondLast, grp);
        KeyFrame kf2 = new KeyFrame(Duration.seconds(0.1), actionEvent -> {
            if (ConvexHullUtil.ccwSlope(secondLast, last, points.get(j)) == 1 || ConvexHullUtil.ccwSlope(secondLast, last, points.get(j)) == 0) {
                stack.push(points.get(j));
                secondLast = last;
                last = points.get(j);
                drawLine(last, secondLast, grp);
            } else {
                grp.getChildren().remove(lines.pop());
                stack.pop();
                last = secondLast;
                secondLast = stack.get(stack.size() - 2);
                j--;
            }
            j++;
            if (j >= points.size()) {
                stack.add(stack.get(0));
                last = stack.get(stack.size() - 1);
                secondLast = stack.get(stack.size() - 2);
                drawLine(last, secondLast, grp);
                tl2.stop();
            }
        });
        tl2.getKeyFrames().add(kf2);
        tl2.play();
    }

    private static void drawLine(point a, point b, Group grp) {
        Line l1 = new Line(a.x, a.y, b.x, b.y);
        l1.setStroke(Color.web("#008000"));
        grp.getChildren().add(l1);
        lines.push(l1);
    }
}
